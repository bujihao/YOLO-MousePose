...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), nkpt=None, ch=(), inplace=True, dw_conv_kpt=False):  # detection layer
        ...
        self.m = nn.ModuleList(nn.Conv2d(x, self.no_det * self.na, 1) for x in ch)  # output conv 对每个输出的feature map都要调用一次conv1x1
        ...
        if self.nkpt is not None:
            if self.dw_conv_kpt: #keypoint head is slightly more complex
                ...
                # self.m_kpt = nn.ModuleList(
                #             nn.Sequential(Conv(x, x, k=3, p=1),
                #                           Conv(x, x, k=3, p=1),
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            bs, _, ny, nx = x[i].shape  # x(bs,81,20,20) to x(bs,3,20,20,27)
            ...
            if not self.training:  # inference
                ...
                if self.inplace:
                    ...
                    if self.nkpt != 0:
                        x_kpt[..., 0::3] = (x_kpt[..., ::3] + kpt_grid_x.repeat(1,1,1,1,7)) * self.stride[i]  # xy
                        x_kpt[..., 1::3] = (x_kpt[..., 1::3] + kpt_grid_y.repeat(1,1,1,1,7)) * self.stride[i]  # xy
                        x_kpt[..., 2::3] = x_kpt[..., 2::3]
                    ...
                    if self.nkpt != 0:
                        y[..., 6:] = (y[..., 6:] + self.grid[i].repeat((1,1,1,1,self.nkpt))) * self.stride[i]  # xy
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        logger.info('GFLOPS input with 640')
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPPF, SPP, DWConv, MixConv2d, Focus, ConvFocus, CrossConv, BottleneckCSP,
                 C3, C3TR, C2f,ConvBNReLU, SE_HALF, ECA_SORT,SE_SORT,InvertedResidual,CBAM,h_sigmoid]:
            ...
            c2 = ch[f] // args[0] ** 2
        elif m is ChannelSelection_Top:
            c2 = args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)
            args = [[ch[f_] for f_ in f], c2]
        elif m is ChannelSelection_Medium:
            c2 = args[0]

            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)
            args = [[ch[f_] for f_ in f], c2]
        elif m is ChannelSelection_Bottom:
            c2 = args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)
            args = [[ch[f_] for f_ in f], c2]
