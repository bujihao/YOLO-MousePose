...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # self.flip_index = [0, 1, 2, 3, 4, 6, 5, 8, 7, 10, 9, 12, 11, 14, 13, 16, 15, 18, 17]
            ...
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        ...
                        if kpt_label:
                            assert l.shape[1] == 26, 'labels require 26 columns each'
                        ...
                        ne += 1  # label empty
                        # print(lb_file)
                        l = np.zeros((0, 7 *2 + 5), dtype=np.float32) if kpt_label else np.zeros((0, 5), dtype=np.float32)
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            # show_i = img.copy()
            # for i in range(7):
            #     cv2.circle(show_i, (int(labels[0, 5 + i * 2]), int(labels[0, 6 + i * 2])), 1, (255, 0, 0), 1)
            # plt.subplot(1, 3, 1)
            # plt.imshow(show_i)
            # print()
...
def load_mosaic(self, index):
    ...
    seq = iaa.Sequential(
        [iaa.arithmetic.CoarseDropout(p=(0.01, 0.05), size_percent=0.3),
         iaa.arithmetic.SaltAndPepper(p=(0.004, 0.02)),
         iaa.blur.GaussianBlur(sigma=(0.001, 0.5))
         ]
    )
    # v = random.uniform(0, 255)
    # transform = A.Compose([
    #     A.GaussianBlur(p=0.8),
    #     A.JpegCompression(quality_lower=20, quality_upper=100),
    #     # A.GridDropout()
    #     A.Cutout(num_holes=200, max_h_size=10, max_w_size=10, fill_value=v, p=0.8),
    # ])
...
def random_perspective(img, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
    ...
    if n:
        ...
        if use_segments:  # warp segments
            ...
            if kpt_label:
                kps = 7
                xy_kpts = np.ones((n * kps, 3))
                xy_kpts[:, :2] = targets[:,5:].reshape(n*kps, 2)  #num_kpt is hardcoded to 17
                xy_kpts = xy_kpts @ M.T # transform
                xy_kpts = (xy_kpts[:, :2] / xy_kpts[:, 2:3] if perspective else xy_kpts[:, :2]).reshape(n, kps*2)  # perspective rescale or affine
                xy_kpts[targets[:,5:]==0] = 0
                x_kpts = xy_kpts[:, list(range(0,kps*2,2))]
                y_kpts = xy_kpts[:, list(range(1,kps*2,2))]
                ...
                xy_kpts[:, list(range(0, kps*2, 2))] = x_kpts
                xy_kpts[:, list(range(1, kps*2, 2))] = y_kpts

        # filter candidates
        i = box_candidates(box1=targets[:, 1:5].T * s, box2=new.T, area_thr=0.01, ar_thr=50, width=width, height=height,
                           ol_thr=0.8)
...
def box_candidates(box1, box2, width, height, wh_thr=2, ar_thr=20, area_thr=0.1, eps=1e-16, ol_thr=0.4):  # box1(4,n), box2(4,n)
    # Compute candidate boxes: box1 before augment, box2 after augment, wh_thr (pixels), aspect_ratio_thr, area_ratio
    w1, h1 = box1[2] - box1[0], box1[3] - box1[1]
    w2, h2 = box2[2] - box2[0], box2[3] - box2[1]
    ar = np.maximum(w2 / (h2 + eps), h2 / (w2 + eps))  # aspect ratio
    x21 = np.clip(box2[0], 0, width)
    x22 = np.clip(box2[2], -1, width)
    y21 = np.clip(box2[1], 0, height)
    y22 = np.clip(box2[3], -1, height)
    ia = (x22 - x21) * (y22 - y21)
    oa = w2 * h2
    return (w2 > wh_thr) & (h2 > wh_thr) & (w2 * h2 / (w1 * h1 + eps) > area_thr) &\
        (ar < ar_thr) & (ia / (oa + eps) > ol_thr) # candidates
